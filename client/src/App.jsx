    ...
    useEffect(() => {
        ...
        for (const trade of openTrades) {
            ...
            if (tslEnabled && riskPerUnit > 0.01 && (trade.action === "BUY" || trade.action === "SELL")) {
                const profit = trade.action === "BUY" ? ltp - spotEntry : spotEntry - ltp;
                ...
                if (profit >= tslTriggerR * riskPerUnit) {
                    ...
                    const shouldMove =
                        trade.action === "BUY" ? newSl > (trade.sl ?? -Infinity) : newSl < (trade.sl ?? Infinity);
            ...
            if (trade.action === "BUY") {
                if (trade.sl != null && ltp <= trade.sl) {
                    closeAt = trade.sl;
                    msg = `${trade.tslActivated ? "🔒" : "🛑"} ${trade.tslActivated ? "TSL" : "SL"} hit — ${
                        trade.symbol
                    } @ ₹${ltp.toFixed(2)}`;
...
function AppShell() {
    ...
    // Settings hidden by default — use ?setting=1 URL param to show

  ...
  return (
    <span
      style={{
        ...
        color, marginLeft: 4, whiteSpace: 'nowrap',
      }}
...
function EqScanRow({ alert }) {
  ...
  return (
    <tr className={`scan-row scan-row--${alert.signal}`}>
      ...
      <td className="scan-cell scan-cell--price">
...
export default function EquityScanPanel({ inline = false }) {
  ...
  const patternOptions = useMemo(() => {
    ...
    return [{ id: 'all', label: 'All patterns' }, ...Array.from(seen.values()).sort((a, b) => a.label.localeCompare(b.label))];
  }, [results]);
  ...
  function renderStatusChip() {
      ...
      if (isPrefetching || progress.total === 0) {
        return (
          <span style={{
            ...
            background: '#f08c001a', color: '#fab005', border: '1px solid #fab00540',
          }}>
    ...
    if (isCached) {
      return (
        <span style={{
          padding: '3px 10px', borderRadius: 12, fontSize: 12, fontWeight: 600,
          background: '#2f9e441a', color: '#51cf66', border: '1px solid #51cf6640',
        }}>
          ...
          {!isRunning && isCached && (
            <button
              ...
              style={{
                ...
                color: 'var(--text-muted)', cursor: 'pointer', textDecoration: 'underline', padding: 0,
              }}
              ...
              <button
                ...
                style={{
                  ...
                  background: signalFilter === s
                    ? (s === 'bullish' ? '#2f9e4433' : s === 'bearish' ? '#e0324233' : 'var(--accent)')
                    : 'var(--bg-tertiary)',
                  color: signalFilter === s
                    ? (s === 'bullish' ? '#51cf66' : s === 'bearish' ? '#ff6b6b' : '#fff')
                    : 'var(--text-secondary)',
      ...
      {isRunning && (
        <div style={{
          ...
          {isPrefetching || progress.total === 0 ? (
            <div style={{
              height: '100%', borderRadius: 2, background: '#fab005',
              width: '40%', animation: 'eq-scan-indeterminate 1.5s ease-in-out infinite',
            }} />
          ) : (
            <div style={{
              height: '100%', borderRadius: 2, background: '#fab005',

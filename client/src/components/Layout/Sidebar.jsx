...
function InstItem({ token, label, sublabel, active, onClick }) {
  ...
  return (
    <div
      className={`sidebar-inst-item ${active ? 'sidebar-inst-item--active' : ''}`}
      onClick={onClick}
    >
...
export default function Sidebar({
  ...
  return (
    <aside className={`sidebar ${collapsed ? 'sidebar--collapsed' : ''} ${isMarket && !collapsed ? 'sidebar--market' : ''}`}>
        ...
        <div className="sidebar-instruments">
          ...
          {indexSidebarItems.map(item => (
            <InstItem
              ...
              onClick={() => selectInstrument({ type: 'index', key: item.key, token: item.token, label: item.label })}
            />
          ))}
          ...
          {MACRO_KEYS.map(m => {
            ...
            return (
              <InstItem
                ...
                onClick={() => selectInstrument({ type: 'macro', key: m.key, token, label: m.label, sublabel: sub })}
              />
            );
          })}
          ...
          <button
            className={`sidebar-manage-btn ${selectedInstrument?.type === 'manage' ? 'sidebar-manage-btn--active' : ''}`}

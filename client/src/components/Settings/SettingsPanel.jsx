      ...
      <div className="settings-panel">
        ...
        <div className="settings-group">
          ...
          <div className="connection-row">
            <div className={`conn-status-btn ${kiteConnected ? 'conn-status-btn--green' : 'conn-status-btn--red'}`}>
              <span className="conn-dot" />
              <span>{kiteConnected ? 'Connected' : 'Not connected'}</span>
            </div>
            {!kiteConnected && (
              <button className="btn btn-primary btn-sm" onClick={openKiteLogin}>Login to Kite →</button>
            )}
          </div>
        ...
        <div className="settings-group">
          <h3>Telegram Bot</h3>
          <div className="connection-row">
            <div className={`conn-status-btn ${isRunning ? 'conn-status-btn--green' : 'conn-status-btn--red'}`}>
              <span className="conn-dot" />
              <span>{isRunning ? 'Polling active' : 'Stopped'}</span>
            </div>
            <button className={`btn btn-sm ${isRunning ? 'btn-danger' : 'btn-success'}`} onClick={togglePolling}>
        ...
        <div className="settings-group">
          ...
          <p className="poll-hint" style={{ marginBottom: '12px' }}>
            Used for every order placed from a signal.
          </p>
          <div className="field-row">
            <div className="field">
              <label>Exchange</label>
              <select value={form.exchange} onChange={(e) => editDefaults('exchange', e.target.value)}>
                <option>NSE</option><option>BSE</option><option>NFO</option><option>MCX</option>
              </select>
            </div>
            <div className="field">
              <label>Product</label>
              <select value={form.product} onChange={(e) => editDefaults('product', e.target.value)}>
            ...
            <div className="field">
              ...
              <input
                ...
                min={1}
                value={form.quantity}
                onChange={(e) => editDefaults('quantity', e.target.value)}
              />
            </div>
          </div>
          {defaultsForm && (
            <button className="btn btn-primary btn-sm" style={{ marginTop: '10px' }} onClick={saveDefaults} disabled={savingDefaults}>

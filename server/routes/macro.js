...
function _analyze(candles, label) {
  ...
  const trend   = ma10 == null ? 'flat'
    ...
    : 'flat';
  return { label, current: Math.round(current * 100) / 100, change, trend, ma10: ma10 != null ? Math.round(ma10 * 100) / 100 : null };
}

function _vixZone(v) {
  return v < 13 ? 'complacency' : v < 16 ? 'calm' : v < 20 ? 'normal' : v < 25 ? 'elevated' : v < 30 ? 'fear' : 'extreme';
}

/**
 * Pure Ichimoku signal — 4 components vote, majority wins.
 * Returns null signal if insufficient candles for Ichimoku.
 */
...
function _frontMonthFutures(query, exchange) {
...
async function _analyzeVix() {
  ...
  return { ...(_direction(tfs)), currentVix: (a1d || a1h || a15m)?.current ?? null, timeframes: tfs };
}

async function _analyzeCrude() {
  const inst = _frontMonthFutures('CRUDEOIL', 'MCX');
...
async function _analyzeUsdinr() {
  const inst = _frontMonthFutures('USDINR', 'CDS');
